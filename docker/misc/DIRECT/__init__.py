...
if True:
    ...
    def solve(
        ...
        # logfilename is changed to 'dummylogfile'
        # wrap function
        fnc = lambda x: objective(x, user_data)[0]
        # wrap input from DIRECT to scipydirect
        res = minimize(fnc, bounds=[(l[i], u[i]) for i in range(len(l))] , nvar=None, args=(), disp=False,
             eps=1e-4,
             maxf=20000,
             maxT=6000,
             algmethod=0,
        ...
        # wrap output from scipydirect to DIRECT
        if not res['success']:
            raise Exception(res['message'])

        return res['x'], res['fun'], res['message']

else:
    # @sascha: the package is bugy. Skip to use scipydirect
    import numpy as np
